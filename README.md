...
平台现在有两条小红书流程，但入口不一样。TikHub 只负责搜索和抓取评论，DeepSeek/模型模块负责分析与总结。

第一条是“TikHub 周报”，从平台页面手动发起：
...
第二条是“单条小红书链接分析”，统一走飞书监听器，不单独做后台页面和配置模块：
...
`数据表` 建议字段：`小红书链接`、`任务状态`、`提交人`、`优化意见`。

`输出表` 和 `ai表格` 建议字段：`原帖链接`、`参考性`、`痛点摘要`、`功效期望`、`成分态度`、`竞品情报`、`价格信号`、`研发建议`、`备注`。

使用单条链接分析时，在“飞书表配置”中把输入表登记为 `小红书链接数据表`，把结果表登记为 `小红书链接分析输出表`，然后在“飞书监听”里新增监听器，监听类型选择 `小红书链接分析`。

小红书分析和周报汇总的 system prompt 可以在“TikHub”页面的“提示词配置”里直接编辑，保存后对后续周报和单条链接分析生效。配置中心的 `TikHub` 只保存 TikHub token、请求 URL 和评论请求参数。

`每周更新总结报告` 最小字段：`标题`、`报告正文`。
